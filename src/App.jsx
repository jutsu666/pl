...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  :root{
    --bg:#0b0b0f; --bg2:#111118; --bg3:#17171f; --card:#141420;
    --border:rgba(255,255,255,0.065); --border2:rgba(255,255,255,0.11);
    --accent:#7c5cfc; --accent2:#5c8bfc; --glow:rgba(124,92,252,0.2);
    --success:#34d399; --danger:#f87171; --warning:#fbbf24;
    --text:#dde0f0; --text2:#8888aa; --text3:#505070;
    --font:'Inter',system-ui,sans-serif; --r:9px; --r2:14px;
  }
  ...
  .pa{animation:fadeUp .28s ease both}
  .card{background:var(--card);border:1px solid var(--border);border-radius:var(--r2);padding:20px 22px}
  .nav-item{display:flex;align-items:center;gap:9px;padding:8px 10px;border-radius:9px;cursor:pointer;font-size:13px;font-weight:500;color:var(--text2);transition:all .15s;margin-bottom:2px;user-select:none}
  .nav-item:hover{background:var(--bg3);color:var(--text)}
  .nav-item.active{background:rgba(124,92,252,0.13);color:#a78bfa}
  .btn-p{display:inline-flex;align-items:center;gap:6px;padding:8px 15px;border-radius:var(--r);border:none;background:var(--accent);color:#fff;font-size:13px;font-weight:600;cursor:pointer;transition:opacity .15s,transform .15s}
  .btn-p:hover{opacity:.88;transform:translateY(-1px)}
  .btn-g{display:inline-flex;align-items:center;gap:6px;padding:7px 12px;border-radius:var(--r);border:1px solid var(--border2);background:var(--bg3);color:var(--text2);font-size:12px;font-weight:500;cursor:pointer;transition:all .15s}
  .btn-g:hover{background:var(--bg2);color:var(--text);border-color:rgba(255,255,255,0.18)}
  .btn-i{display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:7px;border:1px solid var(--border);background:var(--bg3);color:var(--text3);cursor:pointer;transition:all .15s;flex-shrink:0}
  .btn-i:hover{background:var(--bg2);color:var(--text2);border-color:var(--border2)}
  .btn-i.d:hover{border-color:rgba(248,113,113,.35);color:var(--danger);background:rgba(248,113,113,.05)}
  .cnt{display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:7px;border:1px solid var(--border2);background:var(--bg3);color:var(--text);font-size:15px;font-weight:700;cursor:pointer;transition:all .12s;user-select:none;flex-shrink:0}
  .cnt:hover{background:var(--accent);border-color:var(--accent);color:#fff}
  .cnt:active{transform:scale(.9)}
  .fi{width:100%;padding:9px 11px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--r);color:var(--text);font-size:13px;outline:none;transition:border-color .15s}
  .fi:focus{border-color:rgba(124,92,252,.5);box-shadow:0 0 0 3px rgba(124,92,252,.07)}
  th{font-size:10.5px;text-transform:uppercase;letter-spacing:.065em;color:var(--text3);font-weight:600;padding:9px 13px;text-align:left;white-space:nowrap;border-bottom:1px solid var(--border)}
  td{padding:9px 13px;color:var(--text2);border-bottom:1px solid rgba(255,255,255,.03);vertical-align:middle}
...
// ── SHARED COMPONENTS ──
const Lbl = ({ children }) => <label style={{ display: "block", fontSize: 10.5, color: "var(--text3)", marginBottom: 5, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".065em" }}>{children}</label>;
...
const FS = ({ label, options, ...p }) => (
  <Field label={label}>
    <select className="fi" {...p} style={{ cursor: "pointer" }}>
...
function Badge({ status }) {
  const M = { sold: ["Продано", "#34d399", "rgba(52,211,153,.1)"], pending: ["Ожидание", "#fbbf24", "rgba(251,191,36,.1)"], cancelled: ["Отменено", "#f87171", "rgba(248,113,113,.1)"], open: ["Открыт", "#38bdf8", "rgba(56,189,248,.1)"], closed: ["Закрыт", "#50507a", "rgba(80,80,122,.15)"], active: ["Активен", "#34d399", "rgba(52,211,153,.1)"], inactive: ["Неактивен", "#50507a", "rgba(80,80,122,.15)"] };
...
function Modal({ open, onClose, title, children, width = 440 }) {
  ...
  if (!open) return null;
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 600, backdropFilter: "blur(10px)", animation: "fadeIn .18s ease" }}>
      <div style={{ background: "var(--card)", border: "1px solid var(--border2)", borderRadius: 18, padding: "24px 26px", width, maxWidth: "94vw", maxHeight: "92vh", overflowY: "auto", animation: "scaleIn .2s ease", boxShadow: "0 30px 90px rgba(0,0,0,.65), 0 0 0 1px rgba(255,255,255,.03)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
          <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", letterSpacing: "-.01em" }}>{title}</span>
...
function useToast() {
  ...
  const show = useCallback((msg, type = "ok") => { setT({ msg, type }); clearTimeout(r.current); r.current = setTimeout(() => setT(null), 2700); }, []);
  return { toast: t, show };
}
...
function AuthScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "fixed", top: "30%", left: "20%", width: 300, height: 300, borderRadius: "50%", background: "radial-gradient(circle, rgba(124,92,252,.06) 0%, transparent 70%)", pointerEvents: "none" }} />
      <div style={{ width: 390, background: "var(--card)", border: "1px solid var(--border2)", borderRadius: 20, padding: "34px 32px", boxShadow: "0 40px 100px rgba(0,0,0,.6), inset 0 1px 0 rgba(255,255,255,.04)", animation: "scaleIn .3s ease", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", background: "var(--bg2)", borderRadius: 10, padding: 3, marginBottom: 20, gap: 3 }}>
          {[["admin", "👑 Admin"], ["worker", "🔧 Worker"]].map(([m, l]) => (
            <button key={m} onClick={() => { setMode(m); setError(""); }}
              style={{ flex: 1, padding: "7px 0", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 600, fontSize: 13, transition: "all .2s", background: mode === m ? "var(--accent)" : "transparent", color: mode === m ? "#fff" : "var(--text3)", boxShadow: mode === m ? "0 2px 12px rgba(124,92,252,.3)" : "none" }}>
              {l}
            </button>
          ))}
        </div>
        <form onSubmit={mode === "admin" ? handleAdmin : handleWorker}>
          ...
          {error && <div style={{ fontSize: 12, color: "var(--danger)", padding: "8px 11px", background: "rgba(248,113,113,.07)", borderRadius: 8, marginBottom: 12, border: "1px solid rgba(248,113,113,.18)" }}>{error}</div>}
          <button type="submit" disabled={loading} style={{ width: "100%", padding: "11px", borderRadius: "var(--r)", border: "none", background: "linear-gradient(135deg,#7c5cfc,#5c8bfc)", color: "#fff", fontSize: 14, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? .7 : 1, boxShadow: "0 4px 18px rgba(124,92,252,.35)", letterSpacing: ".01em" }}>
...
export default function App() {
  ...
  const loadData = async () => {
    ...
    const [cats, prods, daySheets, orders, bumps, wks] = await Promise.all([
      ...
      isAdmin ? supabase.from("day_sheets").select("*").order("created_at", { ascending: false })
  ...
  const getSheetProfit = useCallback((sheetId) => {
    ...
    return { gross: Math.round(gross), bumps: Math.round(bumps), net: Math.round(gross - bumps) };
  ...
  const updateDay = (productId, field, delta) => {
    ...
    setDailyCounts(upd); lsSet(LS.dailyCounts, upd);
  };
  ...
  const chartData = Array.from({ length: 7 }, (_, i) => {
    ...
    const v = sheetOrders.filter(o => { const od = new Date(o.created_at); return od.getDate() === d.getDate() && od.getMonth() === d.getMonth() && od.getFullYear() === d.getFullYear() && o.status === "sold"; })
  ...
  const catColors = ["#7c5cfc", "#38bdf8", "#34d399", "#fbbf24", "#f87171", "#a78bfa", "#fb923c"];
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <aside style={{ width: 205, background: "var(--bg2)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 100 }}>
          <div style={{ padding: "18px 14px 12px", display: "flex", alignItems: "center", gap: 9 }}>
            ...
            <div><div style={{ fontSize: 13.5, fontWeight: 800, color: "var(--text)", letterSpacing: "-.01em" }}>Playerok</div><div style={{ fontSize: 10, color: "var(--text3)" }}>Tracker</div></div>
          </div>
        ...
        <main style={{ marginLeft: 205, flex: 1, display: "flex", flexDirection: "column" }}>
          {/* TOPBAR */}
          <div style={{ height: 52, borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 22px", background: "rgba(17,17,24,.9)", position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(12px)" }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", letterSpacing: "-.01em" }}>
              {navItems.find(n => n.id === page)?.label}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              <div onClick={() => setPage("rate")} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 8, background: "var(--bg3)", border: "1px solid var(--border)", fontSize: 12, color: "var(--text2)", cursor: "pointer", transition: "all .15s" }}
                onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(124,92,252,.4)"}
          ...
          <div style={{ flex: 1, padding: "18px 22px", overflowY: "auto" }}>
            ...
            {page === "dashboard" && (
              <div className="pa">
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 11, marginBottom: 16 }}>
                  ...
                  <MetricCard label="Итог сегодня" value={fmt(netToday) + " ₽"} sub={totalQtyToday + " продаж · −" + fmt(totalBumpsToday) + " ₽ подн."} accent="#fbbf24" delay={.15} />
                </div>
                  ...
                  <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                    ...
                    <div style={{ display: "flex" }}>
                      {categories.map((cat, i) => {
                        ...
                        const cf = cp.reduce((s, p) => { const e = dayEntry[p.id] || { qty: 0 }; return s + Math.round(calcProfit({ salePrice: p.sale_price_rub, buyUsd: p.purchase_usd, playerokRate, commType: p.commission_type })) * e.qty; }, 0);
                        return (
                  ...
                  <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      <button className="btn-g" style={{ fontSize: 11, padding: "4px 9px" }} onClick={() => {
                        ...
                        const u = { ...dailyCounts }; delete u[dk]; setDailyCounts(u); lsSet(LS.dailyCounts, u); showToast("Счётчики сброшены");
                      }}>Сбросить</button>
                    </div>
                  </div>
                  ...
                  {sheets.map((sh, i) => {
                    ...
                    return (
                      <div key={sh.id} style={{ background: "var(--card)", border: `1px solid ${sh.status === "open" ? "rgba(56,189,248,.22)" : "var(--border)"}`, borderRadius: "var(--r2)", padding: 16, animation: `fadeUp .28s ease ${i * .04}s both`, transition: "transform .2s", position: "relative" }}
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 7, marginBottom: sh.status === "open" && isAdmin ? 11 : 0 }}>
                          {[[cnt, "Продаж", "var(--text)"], [(net >= 0 ? "+" : "") + fmt(net) + " ₽", "Профит", net >= 0 ? "var(--success)" : "var(--danger)"], [bumps ? "−" + fmt(bumps) + " ₽" : "0", "Поднятия", "var(--warning)"]].map(([v, l, c], j) => (
                        ...
                        {isAdmin && sh.worker_id && (
                          <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 9px", background: "var(--bg3)", borderRadius: 7, marginBottom: 9 }}>
                            ...
                            <input type="number" min={0} max={100} value={share}
                              ...
                              style={{ width: 44, padding: "3px 5px", background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 6, color: "var(--text)", fontSize: 12, textAlign: "center" }} />
            ...
            {/* ── CALC ── */}
            {page === "calc" && (() => {
              const [sp, setSp] = useState(""); const [bu, setBu] = useState(""); const [pr, setPr] = useState(playerokRate);
              const [ct, setCt] = useState("standard"); const [ws, setWs] = useState(50); const [bm, setBm] = useState(0); const [pid, setPid] = useState("");
              const sale = +sp || 0; const buy = +bu || 0; const rate = +pr || playerokRate;
              const commRate = ct === "premium" ? 0.04 : 0.05;
              const commAmt = Math.round(sale * commRate); const buyRub = Math.round(buy * rate);
              const gross = sale - commAmt - buyRub; const net = gross - bm;
              const wAmt = ws > 0 ? Math.round(net * ws / 100) : 0; const aAmt = net - wAmt;
              const row = (l, v, c) => (
              ...
              return (
                <div className="pa" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, maxWidth: 820 }}>
                  <div className="card">
                    ...
                    <FS label="Быстрый выбор" value={pid} onChange={e => { setPid(e.target.value); const p = products.find(x => x.id == e.target.value); if (p) { setSp(p.sale_price_rub); setBu(p.purchase_usd); setCt(p.commission_type); } }}
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {[{ id: "all", name: "Все" }, ...categories].map(c => (
                        <button key={c.id} onClick={() => setActiveCat(c.id)}
                          style={{ padding: "5px 12px", borderRadius: 18, fontSize: 12, fontWeight: 500, border: "1px solid", borderColor: activeCat == c.id ? "var(--accent)" : "var(--border)", background: activeCat == c.id ? "rgba(124,92,252,.1)" : "var(--bg3)", color: activeCat == c.id ? "#a78bfa" : "var(--text2)", cursor: "pointer", transition: "all .15s" }}>
                          {c.name}
                        </button>
                      ))}
                    </div>
                        ...
                        {categories.map(c => (
                          <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 9px", background: "var(--bg3)", borderRadius: 7, border: "1px solid var(--border)" }}>
                            ...
                            <button className="btn-i d" style={{ width: 18, height: 18, borderRadius: 4, marginLeft: 2 }} onClick={async () => {
                              ...
                              setCategories(prev => prev.filter(x => x.id !== c.id)); showToast("Категория удалена");
                    ...
                    {filtered.map((p, i) => {
                      ...
                      return (
                        <div key={p.id} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--r2)", padding: 15, animation: `fadeUp .25s ease ${i * .03}s both`, transition: "all .2s" }}
                            ...
                            {isAdmin && (
                              <div style={{ display: "flex", gap: 3 }}>
                                ...
                                <button className="btn-i d" style={{ width: 22, height: 22, borderRadius: 5 }} onClick={async () => {
                                  ...
                                  setProducts(prev => prev.filter(x => x.id !== p.id)); showToast("Товар удалён");
                          ...
                          <div style={{ fontSize: 17, fontWeight: 800, color: "var(--text)", letterSpacing: "-.01em" }}>{fmt(p.sale_price_rub)} ₽</div>
            ...
            {page === "rate" && (() => {
              ...
              const doFetch = async () => {
                ...
                else showToast("Не удалось получить — кеш", "err");
                setFetching(false);
              };
              const block = (title, value, color, children) => (
                <div className="card" style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ fontSize: 34, fontWeight: 800, color: "var(--text)", letterSpacing: "-.03em", marginBottom: 10, background: color, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{value}</div>
                  {children}
                </div>
              );
              return (
                <div className="pa" style={{ maxWidth: 520 }}>
                  ...
                  {block("🎮 КУРС PLAYEROK (ручной)", fmtF(playerokRate) + " ₽", "linear-gradient(135deg,#38bdf8,#34d399)",
                    ...
                    ) : <div style={{ fontSize: 12, color: "var(--text3)" }}>Только admin</div>
                  )}
                  <div className="card">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                        ...
                        return (
                          <div key={wk.id} style={{ display: "flex", alignItems: "center", gap: 9, padding: "9px 12px", background: "var(--bg3)", borderRadius: "var(--r)", border: "1px solid var(--border)" }}>
                            ...
                            <button className="btn-i d" onClick={async () => {
                              ...
                              setWorkers(prev => prev.filter(x => x.id !== wk.id)); showToast("Воркер удалён");
                  ...
                  <div className="card">
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <button className="btn-g" onClick={() => { const b = new Blob([JSON.stringify({ sheets, sheetOrders, workers, products, categories }, null, 2)], { type: "application/json" }); const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = "playerok-backup.json"; a.click(); showToast("Экспорт готов"); }}>↓ Экспорт JSON</button>
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        ...
                        <tbody>
                          {wSheets.map(sh => {
                            ...
                            const wP = Math.round(net * share / 100); const aP = Math.round(net * (100 - share) / 100);
                            const cnt = sheetOrders.filter(o => o.sheet_id === sh.id && o.status === "sold").length;
                            return (
      ...
      {/* Add Order */}
      {(() => {
        const [pid, setPid] = useState(""); const [sp, setSp] = useState(""); const [bm, setBm] = useState(0);
        const [sid, setSid] = useState(""); const [wid, setWid] = useState("");
        const openSheets = sheets.filter(s => s.status === "open");
        useEffect(() => { if (openSheets.length) setSid(openSheets[0].id); }, [modals.addOrder]);
        useEffect(() => { const p = products.find(x => x.id == pid); if (p) setSp(p.sale_price_rub); }, [pid]);
        const submit = async () => {
          if (!pid || !sp || !sid) return;
          const order = { sheet_id: +sid, product_id: +pid, sale_price: +sp, status: "sold" };
          if (supabase) { await supabase.from("sheet_orders").insert([order]); if (+bm > 0) await supabase.from("sheet_bumps").insert([{ sheet_id: +sid, amount: +bm }]); await loadData(); }
          else {
            setSheetOrders(prev => [...prev, { ...order, id: Date.now(), created_at: new Date().toISOString() }]);
            if (+bm > 0) setSheetBumps(prev => [...prev, { sheet_id: +sid, amount: +bm, id: Date.now(), created_at: new Date().toISOString() }]);
          }
          showToast("Продажа добавлена"); closeM("addOrder");
        };
        return (
          <Modal open={!!modals.addOrder} onClose={() => closeM("addOrder")} title="Добавить продажу">
            ...
            <FS label="Лист" value={sid} onChange={e => setSid(e.target.value)} options={openSheets.map(s => ({ value: s.id, label: s.name }))} />
            ...
            <MFoot onClose={() => closeM("addOrder")} onSubmit={submit} label="Добавить" />
          </Modal>
        );
      })()}
      ...
      {/* Add Sheet */}
      {(() => {
        const [name, setName] = useState(todayStr()); const [wid, setWid] = useState("");
        const submit = async () => {
          const s = { name, status: "open", worker_id: wid || null };
        ...
        return (
          <Modal open={!!modals.addSheet} onClose={() => closeM("addSheet")} title="Новый лист">
            <FI label="Название" value={name} onChange={e => setName(e.target.value)} placeholder={todayStr()} />
            {isAdmin && <FS label="Воркер" value={wid} onChange={e => setWid(e.target.value)} options={[{ value: "", label: "Admin (я)" }, ...workers.map(w => ({ value: w.id, label: w.name }))]} />}
            <MFoot onClose={() => closeM("addSheet")} onSubmit={submit} label="Создать" />
          </Modal>
        );
      })()}
      ...
      {(() => {
        ...
        const submit = async () => {
          if (!name || !key) return;
          if (supabase) { await supabase.from("worker_keys").insert([{ name, key, share: +share }]); await loadData(); }
          else setWorkers(prev => [...prev, { id: Date.now(), name, key, share: +share }]);
          showToast("Воркер добавлен"); closeM("addWorker");
        };
        return (
          <Modal open={!!modals.addWorker} onClose={() => closeM("addWorker")} title="Добавить воркера">
            ...
            <MFoot onClose={() => closeM("addWorker")} onSubmit={submit} label="Добавить" />
          </Modal>
        );
      })()}
      ...
      {(() => {
        ...
        const submit = async () => {
          ...
          showToast("Категория добавлена"); closeM("addCategory"); setName("");
        };
        return (
          <Modal open={!!modals.addCategory} onClose={() => closeM("addCategory")} title="Новая категория" width={360}>
            <FI label="Название" value={name} onChange={e => setName(e.target.value)} placeholder="Steam, Valorant..." />
            <MFoot onClose={() => closeM("addCategory")} onSubmit={submit} label="Создать" />
          </Modal>
        );
      })()}
      ...
      {/* Add Product */}
      {(() => {
        const [name, setName] = useState(""); const [catId, setCatId] = useState(""); const [price, setPrice] = useState("");
        const [buyUsd, setBuyUsd] = useState(""); const [ct, setCt] = useState("standard");
        const submit = async () => {
          if (!name || !price) return;
          const p = { name, category_id: catId || null, sale_price_rub: +price, purchase_usd: +buyUsd || 0, commission_type: ct, status: "active" };
          ...
          else setProducts(prev => [...prev, { ...p, id: Date.now(), category: categories.find(c => c.id == catId)?.name || "" }]);
          showToast("Товар добавлен"); closeM("addProduct");
        };
        return (
          <Modal open={!!modals.addProduct} onClose={() => closeM("addProduct")} title="Добавить товар">
            ...
            <MFoot onClose={() => closeM("addProduct")} onSubmit={submit} label="Добавить" />
          </Modal>
        );
      })()}
      ...
      {editTarget && (() => {
        const p = editTarget;
        const [name, setName] = useState(p.name); const [catId, setCatId] = useState(p.category_id || "");
        const [price, setPrice] = useState(p.sale_price_rub); const [buyUsd, setBuyUsd] = useState(p.purchase_usd);
        const [ct, setCt] = useState(p.commission_type || "standard"); const [status, setStatus] = useState(p.status || "active");
        const submit = async () => {
          const upd = { name, category_id: catId || null, sale_price_rub: +price, purchase_usd: +buyUsd || 0, commission_type: ct, status };
          if (supabase) await supabase.from("products").update(upd).eq("id", p.id);
          setProducts(prev => prev.map(x => x.id === p.id ? { ...x, ...upd, category: categories.find(c => c.id == catId)?.name || x.category } : x));
          showToast("Товар обновлён"); closeM("editProduct"); setEditTarget(null);
        };
        return (
          <Modal open={!!modals.editProduct} onClose={() => { closeM("editProduct"); setEditTarget(null); }} title={"Редактировать: " + p.name}>
            ...
            <MFoot onClose={() => { closeM("editProduct"); setEditTarget(null); }} onSubmit={submit} label="Сохранить" />
          </Modal>
        );
      })()}
